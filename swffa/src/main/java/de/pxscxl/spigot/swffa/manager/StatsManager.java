...
import de.pxscxl.spigot.swffa.utils.enums.Rank;
...
public class StatsManager {
    ...
    public StatsManager() {
        ...
        create = session.prepareUpdateStatement("CREATE TABLE IF NOT EXISTS skywarsffa_stats (uuid VARCHAR(36), stats TEXT)");
        insert = session.prepareUpdateStatement("INSERT INTO skywarsffa_stats (uuid, stats) VALUES (?, ?)");
        exist = session.prepareQueryStatement("SELECT uuid FROM skywarsffa_stats WHERE uuid = ?");

        getStats = session.prepareQueryStatement("SELECT * FROM skywarsffa_stats WHERE uuid = ?");
        setStats = session.prepareUpdateStatement("UPDATE skywarsffa_stats SET stats = ? WHERE uuid = ?");

        getAllStats = session.prepareQueryStatement("SELECT * FROM skywarsffa_stats");
    ...
    public void runWrite(Runnable runnable) {
        Bukkit.getScheduler().runTaskAsynchronously(SkyWarsFFA.getInstance(), () -> {
